...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.arrayPositions = [NSArray arrayWithObjects:
                               [NSDictionary dictionaryWithObjectsAndKeys:@"0", kImagePositionx, @"10", kImagePositiony, nil],
                               [NSDictionary dictionaryWithObjectsAndKeys:@"76", kImagePositionx, @"10", kImagePositiony, nil],
                               [NSDictionary dictionaryWithObjectsAndKeys:@"152", kImagePositionx, @"10", kImagePositiony, nil], nil];
...
- (void)setPhotos:(NSArray*)photos
{
    ...
    for (int i=0; i<count; i++) {
        NSDictionary *dictionaryTemp = [self.arrayPositions objectAtIndex:i%3];
        CGFloat originx = [[dictionaryTemp objectForKey:kImagePositionx] floatValue];
        CGFloat originy = [[dictionaryTemp objectForKey:kImagePositiony] floatValue] +(i / 3 )* kFrameHeight ;
    ...
    NSDictionary *dictionaryTemp = [self.arrayPositions objectAtIndex:count%3] ;
    CGFloat originx = [[dictionaryTemp objectForKey:kImagePositionx] floatValue] ;
    CGFloat originy = [[dictionaryTemp objectForKey:kImagePositiony] floatValue]  + (count / 3 )* kFrameHeight ;
    HGPhoto *photoTemp = [[HGPhoto alloc] initWithOrigin:CGPointMake(originx, originy)];
    photoTemp.delegate = self;
...
- (void)addPhoto:(NSString*)string
{
    ...
    NSDictionary *dictionaryTemp = [self.arrayPositions objectAtIndex:index % 3];
    CGFloat originx = [[dictionaryTemp objectForKey:kImagePositionx] floatValue];
    CGFloat originy = [[dictionaryTemp objectForKey:kImagePositiony] floatValue]  + (index / 3 )* kFrameHeight ;
...
- (void)addPhotoImage:(UIImage *)image withUserData:(NSMutableDictionary *) userData{
    ...
    HGPhoto *photoTemp = [[HGPhoto alloc] initWithOrigin:CGPointMake(originx, originy)];
    photoTemp.delegate = self;
    photoTemp.userData = userData;
    
    photoTemp.userNameLabel.text = [userData objectForKey:@"name"];
...
- (void)reloadPhotos:(BOOL)add
{
    ...
    for (int i=0; i<count; i++) {
        ...
        if (i%2 == 0) {
            photoTemp.backgroundColor = [UIColor redColor];
        }else{
            photoTemp.backgroundColor = [UIColor greenColor];
        }
    }
